function minCadence = addatter_rms_to_cadence_lc(campaign, tweakBad, threshold, persistence, singleThreshold)
%NAME:
%  addatter_rms_to_cadence_lc.m
%PURPOSE:
...
grid
saveas(gcf,['c' campaign '_addatterr_mjd_lc.fig'])
saveas(gcf,['c' campaign '_addatterr_mjd_lc.png'])
%%
figure('Position',[1 1 1200 500])
...
grid on
saveas(gcf,['c' campaign '_adatterr2pix_mjd_lc.fig'])
saveas(gcf,['c' campaign '_adatterr2pix_mjd_lc.png'])
%%
% convert MJD to cadence using Equation of Time
load(['c' campaign '_eot_lc.mat'])
excludeBufferSize = 1;
...
save(['c' campaign '_max_rms_lc.mat'], 'cadencesWithNoEngData', 'listOfCadences','maxErr','campaign','excludeBufferSize')
%%
baseCadence = floor(listOfCadences(1)/1000)*1000;
momentumDumps = dlmread(['c' campaign '_isMmntmDmp_LC.txt']);
adcsNotFinePoint = dlmread(['c' campaign '_isNotFinePoint_LC.txt']);
...
xlabel(['LC Cadence - ' num2str(baseCadence)],'FontSize',12)
ylabel('Science Pixels','FontSize',12)
grid on
hold on
...
fid = fopen(['c' campaign '_coarse_point_DAF_LC_ADATTER_' num2str(threshold) '_' ...
    num2str(persistence) '.xml'],'w');
fprintf(fid,'ADATTERR-derived LC COARSE_POINT, program addatter_rms_to_cadence_lc.m\n');
...
sprintf(' <dataAnomaly type="COARSE_POINT" cadenceType="LONG" startCadence="%5.0f" endCadence="%5.0f"/>\n',...
...
fprintf(fid, ' <dataAnomaly type="COARSE_POINT" cadenceType="LONG" startCadence="%5.0f" endCadence="%5.0f"/>\n',...
     max(minCadence, listOfCadences(startGaps(bigGaps(i))) - excludeBufferSize), ...
     min(maxCadence, listOfCadences(lastInGaps(bigGaps(i))) + excludeBufferSize));
end
...
saveas(gcf,['c' campaign '_addatterr2pix_cadence_nodump_' num2str(threshold) '_' ...
    num2str(persistence) '_lc.fig'])
saveas(gcf,['c' campaign '_addatterr2pix_cadence_nodump_' num2str(threshold) '_' ...
    num2str(persistence) '_lc.png'])

save(['c' campaign '_lc_excluded_cadences.mat'],'listOfCadences','listOfExcludedCadences')
fclose(fid);
end
